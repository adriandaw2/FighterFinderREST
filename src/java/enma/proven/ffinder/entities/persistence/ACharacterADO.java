...
public class ACharacterADO {
    ...
    static final String GET_GAME_NO_CHARACTER_USER = "SELECT c.id, c.name FROM `character` c JOIN `user_character` uc ON c.id NOT IN (SELECT ucs.character_id FROM `user_character` ucs WHERE ucs.user_id = ?) WHERE c.id_game = ? ORDER BY c.name";
    static final String GET_GAME_CHARACTER_USER = "SELECT c.id, c.name FROM `character` c JOIN `user_character` uc ON c.id IN (SELECT ucs.character_id FROM `user_character` ucs WHERE ucs.user_id = ?) WHERE c.id_game = ? ORDER BY c.name";
